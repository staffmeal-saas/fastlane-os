...
class NhostAuthClient {
  ...
  async signInEmailPassword(
    email: string,
    password: string,
  ): Promise<{ session: NhostSession | null; error: NhostError | null }> {
    try {
  ...
  async sendResetPasswordEmail(
    ...
    try {
      const res = await fetch(`${this.baseUrl}/user/password/reset`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          email,
...
class NhostStorageClient {
  ...
  async upload(
    ...
    accessToken: string,
    bucketId = "default",
  ): Promise<{ data: UploadResult | null; error: NhostError | null }> {
