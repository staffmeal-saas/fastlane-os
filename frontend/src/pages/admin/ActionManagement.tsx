...
export default function ActionManagement() {
  ...
  const { execute: updateActionStatus } = useLazyGraphQL(
    `mutation($id: uuid!, $status: action_status!, $oldStatus: String, $newStatus: String!) {
      update_actions_by_pk(pk_columns: {id: $id}, _set: {status: $status}) { id }
      insert_action_status_history_one(object: {
        action_id: $id, old_status: $oldStatus, new_status: $newStatus
      }) { id }
    }`,
  );
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      console.error(err);
      toast("Erreur de mise a jour du statut", "error");
    }
  };
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          ...
          >
            <div className="modal-header">
              <h3 className="modal-title">Nouvelle action</h3>
              <button
            ...
            <div className="form-group">
              ...
              <input
                className="form-input"
                placeholder="Ex: Rédaction script..."
                value={form.title}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
            ...
            >
              ...
              <div className="form-group">
                <label className="form-label">Crédits réservés</label>
                <input
                  className="form-input"
                  type="number"
                  placeholder="Ex: 80"
                  value={form.credits_reserved}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    setForm({ ...form, credits_reserved: e.target.value })
                  }
                />
              </div>
            </div>
