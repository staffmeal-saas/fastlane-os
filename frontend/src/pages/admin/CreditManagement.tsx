...
import { CreditCard, Plus, Search, ArrowUpRight, ArrowDownRight, RotateCcw, CheckCircle2, XCircle, Clock } from 'lucide-react'
...
interface CreditData {
  ...
  upgrades: Array<{
    id: string; credits_amount: number; pack_type: string; status: UpgradeStatus
    allocation_target?: string; created_at: string
...
const upgradeStatusLabels: Record<UpgradeStatus, string> = { brouillon: 'Brouillon', demande: 'Demande', valide: 'Validé', paye: 'Payé', credite: 'Crédité', annule: 'Annulé' }
const upgradeStatusColors: Record<UpgradeStatus, string> = { brouillon: 'badge-warning', demande: 'badge-info', valide: 'badge-primary', paye: 'badge-purple', credite: 'badge-success', annule: 'badge-danger' }

export default function CreditManagement() {
  const { session } = useAuth()
  const [search, setSearch] = useState('')
  const [showAdjust, setShowAdjust] = useState(false)
  const [adjustForm, setAdjustForm] = useState<AdjustForm>({ client_id: '', amount: 0, description: '' })
  ...
  const handleUpgradeStatus = async (upgradeId: string, newStatus: UpgradeStatus) => {
    try {
  ...
  const handleValidateAndCredit = async (upgrade: CreditData['upgrades'][0]) => {
  ...
  const pendingUpgrades = upgrades.filter(u => u.status === 'demande' || u.status === 'valide' || u.status === 'paye')
  ...
  return (
    <div className="animate-in">
      <div className="page-header">
        <div><h1 className="page-title">Gestion Crédits</h1><p className="page-subtitle">Allocations, ajustements, recharges et suivi global.</p></div>
      ...
      <div style={{ display: 'flex', gap: 'var(--space-sm)', marginBottom: 'var(--space-xl)' }}>
        <button className={`btn ${tab === 'wallets' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setTab('wallets')}>Wallets</button>
        <button className={`btn ${tab === 'upgrades' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setTab('upgrades')}>Recharges {pendingUpgrades.length > 0 && <span style={{ marginLeft: 6, background: 'var(--color-danger)', color: 'white', borderRadius: 10, padding: '1px 6px', fontSize: '0.7rem' }}>{pendingUpgrades.length}</span>}</button>
      </div>
      ...
      {tab === 'upgrades' && (
        <div className="card">
          <table className="data-table">
            <thead><tr><th>Client</th><th>Pack</th><th>Crédits</th><th>Statut</th><th>Date</th><th>Actions</th></tr></thead>
            <tbody>
              {upgrades.map(u => (
                <tr key={u.id}>
                  <td style={{ fontWeight: 700 }}>{u.client?.name || '—'}</td>
                  <td><span className="badge badge-info">Pack {u.pack_type}</span></td>
                  <td style={{ fontVariantNumeric: 'tabular-nums', fontWeight: 600 }}>+{u.credits_amount}</td>
                  <td><span className={`badge ${upgradeStatusColors[u.status] || 'badge-info'}`}>{upgradeStatusLabels[u.status] || u.status}</span></td>
                  <td>{new Date(u.created_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}</td>
                  <td>
                    <div style={{ display: 'flex', gap: 'var(--space-xs)' }}>
                      {u.status === 'demande' && (
                        <>
                          <button className="btn btn-sm btn-primary" onClick={() => handleUpgradeStatus(u.id, 'valide')} title="Valider"><CheckCircle2 size={14} /></button>
                          <button className="btn btn-sm btn-ghost" onClick={() => handleUpgradeStatus(u.id, 'annule')} title="Refuser" style={{ color: 'var(--color-danger)' }}><XCircle size={14} /></button>
                        </>
                      )}
                      {u.status === 'valide' && (
                        <button className="btn btn-sm btn-primary" onClick={() => handleUpgradeStatus(u.id, 'paye')} title="Confirmer paiement"><CheckCircle2 size={14} /></button>
                      )}
                      {u.status === 'paye' && (
                        <button className="btn btn-sm btn-primary" onClick={() => handleValidateAndCredit(u)} title="Créditer le wallet">
                          <ArrowUpRight size={14} /> Créditer
                        </button>
                      )}
                      {(u.status === 'credite' || u.status === 'annule') && <span style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)' }}>—</span>}
                    </div>
                  </td>
                </tr>
              ))}
              {upgrades.length === 0 && <tr><td colSpan={6} style={{ textAlign: 'center', padding: 'var(--space-xl)', color: 'var(--color-text-muted)' }}>Aucune demande de recharge.</td></tr>}
            </tbody>
          </table>
        </div>
      )}
      ...
      {showAdjust && (
        <div className="modal-overlay" onClick={() => setShowAdjust(false)}>
          <div className="modal" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
            ...
            <div className="form-group"><label className="form-label">Client</label><select className="form-select" value={adjustForm.client_id} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setAdjustForm({...adjustForm, client_id: e.target.value})}><option value="">Sélectionner...</option>{clients.filter(c => c.wallet).map(c => <option key={c.id} value={c.id}>{c.name} (solde: {c.wallet?.balance || 0})</option>)}</select></div>
            <div className="form-group"><label className="form-label">Montant (positif = crédit, négatif = débit)</label><input className="form-input" type="number" placeholder="Ex: 200 ou -50" value={adjustForm.amount} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setAdjustForm({...adjustForm, amount: e.target.value})} /></div>
            <div className="form-group"><label className="form-label">Motif</label><textarea className="form-textarea" placeholder="Geste commercial, correction, etc." value={adjustForm.description} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setAdjustForm({...adjustForm, description: e.target.value})} /></div>
