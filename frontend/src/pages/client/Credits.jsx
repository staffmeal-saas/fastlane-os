...
const typeLabels = { allocation: 'Allocation', consumption: 'Consommation', reservation: 'Réservation', release: 'Libération', carry_over: 'Report', recharge: 'Recharge', adjustment: 'Ajustement', expiration: 'Expiration' }
const typeColors = { allocation: 'badge-success', consumption: 'badge-danger', reservation: 'badge-warning', release: 'badge-info', carry_over: 'badge-purple', recharge: 'badge-success', adjustment: 'badge-info', expiration: 'badge-danger' }

export default function Credits() {
  const { session, currentWallet } = useAuth()
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      try {
        ...
        if (data?.wallet_transactions) setTransactions(data.wallet_transactions)
  ...
  return (
    <div className="animate-in">
      <div className="page-header">
        ...
        <button className="btn btn-primary"><Plus size={16} /> Recharger</button>
      </div>
      ...
      <div className="card">
        <div className="card-header">
          ...
          <div style={{ display: 'flex', gap: 'var(--space-sm)' }}>
            <select className="form-select" style={{ width: 'auto', padding: '4px 12px', fontSize: '0.8rem' }} value={filter} onChange={e => setFilter(e.target.value)}>
