...
import {
  ...
  Zap, BarChart3, CheckCircle2, Clock
} from 'lucide-react'
...
export default function Dashboard() {
  const { currentClient, currentWallet, session, isAdmin } = useAuth()
  const [campaigns, setCampaigns] = useState([])
  const [actions, setActions] = useState([])
  const [documents, setDocuments] = useState([])
  const [stats, setStats] = useState({ campaigns: 0, actions: 0 })
    ...
    <div className="animate-in">
      ...
      <div className="stats-grid">
        {[
          ...
          <div key={i} className="stat-card" style={{ '--stat-bg': stat.bg, '--stat-color': stat.color, animationDelay: `${i * 0.1}s` }}>
