...
import { Zap, Target, TrendingUp, Users, Megaphone, BarChart3, FileText, ArrowRight } from 'lucide-react'
import { Link } from 'react-router-dom'
import { useAuth } from '../../contexts/AuthContext'
import nhost from '../../lib/nhost'

const categoryIcons = { acquisition: Users, conversion: Target, events: Megaphone, automation: Zap, sales_enablement: BarChart3 }
const categoryColors = { acquisition: 'var(--color-primary)', conversion: 'var(--color-success)', events: 'var(--color-warning)', automation: 'var(--color-purple)', sales_enablement: 'var(--color-info)' }
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      try {
        ...
        if (data?.strategic_blocks) setBlocks(data.strategic_blocks)
      ...
      {blocks.length > 0 ? (
        <div className="section-grid">
          {blocks.map(block => {
            ...
            const kpis = block.kpi_targets ? (typeof block.kpi_targets === 'object' ? Object.entries(block.kpi_targets) : []) : []
            return (
