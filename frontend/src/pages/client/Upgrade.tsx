...
import { ArrowUpCircle, CreditCard, CheckCircle2, Wallet, Megaphone, LucideIcon } from 'lucide-react'
import { useAuth } from '../../contexts/AuthContext'
import { useLazyGraphQL } from '../../hooks/useGraphQL'
...
export default function Upgrade() {
  const { currentClient, currentWallet, session } = useAuth()
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-sm)' }}>
              {allocationOptions.map(opt => (
                <label key={opt.value} style={{
                  ...
                  <input type="radio" name="allocation" value={opt.value} checked={allocation === opt.value} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setAllocation(e.target.value)} style={{ display: 'none' }} />
