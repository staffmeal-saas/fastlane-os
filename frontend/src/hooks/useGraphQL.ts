...
export function useLazyGraphQL<T = unknown>(
  query: string,
): UseLazyGraphQLResult<T> {
  const { session, nhost } = useAuth();
  const [data, setData] = useState<T | null>(null);
  ...
  const execute = useCallback(
    async (variables?: Record<string, unknown>) => {
      ...
      try {
        const { data: result, error: gqlError } =
          await nhost.graphql.request<T>({ query, variables }, token, false);
        if (gqlError?.length) {
          setError(gqlError[0].message);
          return null;
        }
