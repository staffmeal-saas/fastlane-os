...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [profile, setProfile] = useState<
    (UserProfile & { clientMemberships: ClientMembership[] }) | null
  >(null);
  ...
  const signIn = async (
    email: string,
    password: string,
  ): Promise<{ error: NhostError | null }> => {
    const { session: newSession, error } = await nhost.auth.signInEmailPassword(
      email,
      password,
    );
  ...
  const signUp = async (
    ...
    const { session: newSession, error } = await nhost.auth.signUpEmailPassword(
      email,
      password,
      displayName,
    );
  ...
  const isClient = profile?.platform_role
    ? ["client_owner", "client_collaborator", "prospect_sprint"].includes(
        profile.platform_role,
      )
    : false;

  const currentClient: Client | null =
    profile?.clientMemberships?.[0]?.client || null;
